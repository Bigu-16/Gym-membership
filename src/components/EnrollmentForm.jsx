...
const EnrollmentForm = ({ onEnroll }) => {
  ...
  const [payment, setPayment] = useState({ amount: '', method: 'Cash', status: 'Paid', currency: 'AED' });
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      setPayment({ amount: '', method: 'Cash', status: 'Paid', currency: 'AED' });
    }, 3000);
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] glass-card p-12 text-center animate-in fade-in zoom-in duration-500">
        ...
        <p className="text-[var(--text-secondary)] text-lg max-w-md">
          The registration for <span className="text-[var(--text-primary)] font-bold">{parentInfo.name}'s</span> family has been processed.
        </p>
      </div>
    );
  }
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {family.trainees.map((trainee, tIndex) => (
                    <div key={tIndex} className="glass-card p-4 flex gap-4 items-center group">
                      <div className="flex-grow grid grid-cols-2 gap-3">
                        <input 
                          ...
                          className="bg-transparent border-b border-[var(--glass-border)] py-1 text-sm focus:outline-none focus:border-[var(--text-primary)] transition-all"
        ...
        {trainingType === 'group' ? (
          <div className="space-y-6">
            <div className="flex justify-between items-end">
              <label className="text-[10px] uppercase tracking-luxury text-[var(--text-secondary)] ml-1">Available Time Slots (Gym)</label>
              <span className="text-[9px] uppercase tracking-widest text-[var(--text-secondary)] opacity-50">Click to select a slot</span>
            </div>
              ...
              {GROUP_SCHEDULE_SLOTS.map(slot => {
                ...
                return (
                  <div 
                    ...
                    className={`slot-pill flex flex-col items-center justify-center py-4 px-2 relative ${schedule.slot === slotText ? 'active' : ''} ${isFull ? 'opacity-40 cursor-not-allowed grayscale' : ''}`}
                  >
