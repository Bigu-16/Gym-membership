    ...
    if (!member.isFrozen) {
      ...
      if (member.isGroup) {
        onUpdateMember({
          ...member,
          ...frozenData,
          trainees: member.trainees.map(t => ({ ...t, ...frozenData }))
        });
      } else {
        onUpdateMember({ ...member, ...frozenData });
      }
    } else {
      const unfrozenData = {
        isFrozen: false,
      ...
      if (member.isGroup) {
        onUpdateMember({
          ...member,
          ...unfrozenData,
          trainees: member.trainees.map(t => ({ ...t, ...unfrozenData }))
        });
      } else {
        onUpdateMember({ ...member, ...unfrozenData });
      }
    }
              ...
              <div className="glass-card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-sm font-semibold">{member.parentName}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase tracking-luxury text-[var(--text-secondary)] mb-1">Phone Number</p>
                    <p className="text-sm font-semibold">{member.parentPhone}</p>
                  </div>
                </div>
              </div>

              {/* Trainees List */}
              <h3 className="text-xs uppercase tracking-luxury text-[var(--text-secondary)] font-bold pt-4">Enrolled Kids ({member.trainees.length})</h3>
              {member.trainees.map((trainee, index) => (
                <div key={trainee.id} className="glass-card p-6">
                   <h4 className="text-[12px] uppercase tracking-luxury text-[var(--text-primary)] font-bold mb-4 flex items-center gap-2">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div>
                       <p className="text-[9px] uppercase tracking-luxury text-[var(--text-secondary)] mb-1">Age / Gender</p>
                       <p className="text-sm font-semibold">{trainee.age || 'N/A'} / {trainee.gender || 'N/A'}</p>
                     </div>
                     <div>
                       <p className="text-[9px] uppercase tracking-luxury text-[var(--text-secondary)] mb-1">Training Type</p>
                       <p className="text-sm font-semibold capitalize">{trainee.trainingType || 'N/A'}</p>
                     </div>
                     <div>
                     ...
                     <div>
                       <p className="text-[9px] uppercase tracking-luxury text-[var(--text-secondary)] mb-1">Schedule</p>
                       <p className="text-sm font-semibold">{trainee.schedule?.slot || 'N/A'}</p>
                     </div>
                   </div>
                </div>
              ))}
