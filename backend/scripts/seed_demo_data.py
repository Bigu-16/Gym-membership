...
async def seed_plans(session) -> dict[str, MembershipPlan]:
    plans = {
...
async def seed_members(session, plans: dict[str, MembershipPlan]) -> dict[str, Member]:
    today = date.today()
    member_rows = [
        {
...
async def seed_schedule(session) -> tuple[dict[str, ScheduleTemplate], dict[str, Session]]:
    today = date.today()
    template_rows = [
        {
...
async def seed_enrollments(session, members: dict[str, Member], templates, sessions) -> None:
    rows = [
...
async def seed_check_ins(session, members: dict[str, Member]) -> None:
    today = date.today()
...
async def seed_notifications(session, members: dict[str, Member]) -> None:
    now = datetime.now(timezone.utc)
    rows = [
        {
            "member": "Marcus Thorne",
